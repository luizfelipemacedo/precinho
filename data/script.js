...
async function scrapeData(searchTerm) {
    ...
    const extrabomProducts = await page.$$eval('.name-produto', elements => elements.map(el => el.innerText));
    const extrabomPrices = await page.$$eval('.item-por__val', elements => elements.map(el => el.innerText));
    const extrabomImages = await page.$$eval('.carousel__img', imgs => imgs.map(img => img.getAttribute('src')));
    ...
    const caroneProducts = await page.$$eval('.product-name', elements => elements.map(el => el.innerText));
    const caronePrices = await page.$$eval('.skuBestPrice', elements => elements.map(el => el.innerText));
    const caroneImages = await page.$$eval('div.product-image img', imgs => imgs.map(img => img.getAttribute('src')));
...
(async () => {
    const searchTerms = ['leite 1L', 'arroz kg', 'feijao kg', 'cafe 500g', 'acucar kg', 'sal refinado', 'oleo de soja', 'farinha de mandioca', 'farinha de trigo', 'macarrao instantaneo'];
