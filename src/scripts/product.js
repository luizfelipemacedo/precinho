...
(async () => {
  ...
  distance =
    distance == null
      ? ""
      : "Estabelecimento mais próximo à " + distance.toFixed(1) + " Km";

  const image = document.querySelector(
    "#detalhes-produto > div.product-image-area > img.product-image"
  );
  image.src = product[0].image;

  const titulo = document.querySelector(
    "#detalhes-produto > div.product-info > h1.product-title"
  );
  titulo.textContent = product[0].name;

  const preco = document.querySelector(
    "#detalhes-produto > div.product-info > p.product-price"
  );
  preco.textContent = `R$${product[0].price.toLocaleString("pt-BR")}`;

  const market = document.querySelector(
    "#detalhes-produto > div.product-info > p.product-market"
  );
  market.innerHTML = `Preço mais baixo no supermecado ${product[0].market}`;

  const marketDistance = document.querySelector(
    "#detalhes-produto > div.product-info > p.product-market.distance"
  );
  marketDistance.textContent = `${distance}`;

  const location = JSON.parse(localStorage.getItem("location"));
  const marketLocation = document.querySelector(
    "#detalhes-produto > div.product-info .map-button"
  );

  const locationRef = location
    ? `@${location.data.lat},${location.data.lon}`
    : ``;
  marketLocation.href = `https://www.google.com.br/maps/search/${product[0].market}+Supermercado/${locationRef}`;

  const favorites = JSON.parse(localStorage.getItem("favorites")) || [];

  const isFav = favorites.includes(product[0].id);
...
async function getRelatedProduts() {
  const data = await fetchData();
  const productName = data.find((product) => product.id == productID).name;

  const [productFirstName] = productName.split(" ");

  const relatedProducts = data
    .filter((product) => product.name.includes(productFirstName))
    .slice(0, 10);

  const duplicateProductIndex = relatedProducts.findIndex(
  ...
  relatedProducts.forEach((product) => {
    const { id, market, name, price, image } = product;
...
function scrollFunction() {
  ...
  if (
    document.body.scrollTop > scrollAmmount ||
    document.documentElement.scrollTop > scrollAmmount
  ) {
